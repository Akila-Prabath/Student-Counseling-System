...
function ShowAppointments() {
  ...
  const reschedule = (appt) => {
    ...
    setForm({
      counselorId: appt.counselor?._id || "",
  ...
  const handleSave = async () => {
    try {
      await API.put(`/appointments/${selectedAppt._id}/reschedule`, form);
  ...
  return (
    <div className="min-h-screen bg-orange-100 p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold mt-2">My Appointments</h2>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            {list.map((appt) => (
              ...
              >
                    ...
                    <img
                      src={
                        appt.counselor?.profilePic
                          ? `${process.env.REACT_APP_API_URL || "http://localhost:8070"}/uploads/${appt.counselor.profilePic}`
                          : `https://ui-avatars.com/api/?name=${encodeURIComponent(appt.counselor?.name || "Counselor")}`
                      }
                      alt="counselor"
                ...
                {/* STATUS */}
                <span
                  className={`inline-block mt-3 text-xs px-3 py-1 rounded-full ${getStatusColor(appt.status)}`}
                >
                  {appt.status}
                </span>
